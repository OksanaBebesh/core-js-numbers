...
function roundToPowerOfTen(num, pow) {
  ...
  result = Math.round(num / digitInPow) * digitInPow;
  return result;
}
...
function isPrime(n) {
  let countDev = 2;
  let isPrimeDigit;
  if (n <= 2) isPrimeDigit = false;
  for (let i = 2; i <= Math.round(Math.sqrt(n)); i += 1) {
    if (n % i === 0) countDev += 1;
  }
  isPrimeDigit = !(countDev > 2);
...
function getSumOfDigits(num) {
  return num
    .toString()
    .split('')
    .reduce((acum, item) => Number(acum) + Number(item));
}
